...
function notify(text)
    local frame = GuiLibrary["CreateNotification"]("Client Loaded", text, 5, "assets/WarningNotification.png")
    frame.Frame.Frame.ImageColor3 = Color3.fromRGB(255, 64, 64)
end
function boxnotify(text)
    if messagebox then
        messagebox(text, "ape v5.74", 0)
     end
end


notify("You have successfullly loaded the module.")
...
local CustomConfig = LIB("Render", {
    ...
    Default = false,
    HoverText = "devs too lazy to maek so yeah"
})
...
local DupeItems = LIB("Utility", {
    ...
    Function = function(callback) 
        if callback then
		notify("error code 72 | if you belive this is a mistake, then contact the developers")
        end
    end,
    Default = false,
...
game.ReplicatedStorage.Items.emerald_chestplate:Clone().Parent = game.ReplicatedStorage.Inventories[lplr.Name]
    ...
    Default = false,
    HoverText = "scripted by dawned_person#3568"
})

local VoidArmour = LIB("Utility", {
    Name = "VoidSet",
    Function = function(callback) 
        if callback then
		local lplr = game.Players.LocalPlayer
...
game.ReplicatedStorage.Items.void_helmet:Clone().Parent = game.ReplicatedStorage.Inventories[lplr.Name]
game.ReplicatedStorage.Items.void_boots:Clone().Parent = game.ReplicatedStorage.Inventories[lplr.Name]
game.ReplicatedStorage.Items.void_chestplate:Clone().Parent = game.ReplicatedStorage.Inventories[lplr.Name]
        end
    end,
    Default = false,
    HoverText = "scripted by dawned_person#3568 thank me kid"
})
...
local ChatCrasher = LIB("Utility", {
    ...
    Default = false,
    HoverText = "stop begging for it"
})

local InfKills = LIB("Render", {
    Name = "VisiualCustomKills",
    Function = function(callback) 
        if callback then
...
local ClientAnticheatDisabler = LIB("Utility", {
    ...
    Default = false,
    HoverText = "Deletes root parts, client only :) YOU THOUGHTTT"
})

LIB("Blatant", {
    Name = "GravityFlyBypassAC_B",
	HoverText = "meant for ac B)",
    Function = function(v)
        longjumpval = v
        if longjumpval then
			workspace.Gravity = 0
